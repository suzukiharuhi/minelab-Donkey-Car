...
VALID_SPEEDS = {"very_slow", "slow", "mid", "high"}

# PWM値テーブル
THROTTLE_FWD = {"very_slow": 1600, "slow": 1670, "mid": 1800, "high": 1900}
THROTTLE_REV = {"very_slow": 1450, "slow": 1300, "mid": 1200, "high": 1100}
YAW_LEFT = {"very_slow": 1320, "slow": 1250, "mid": 1180, "high": 1100}
YAW_RIGHT = {"very_slow": 1720, "slow": 1750, "mid": 1820, "high": 1900}
...
class MotorOutputController:
    ...
    def _normalize_speed(speed: str) -> str:
        ...
        - "very_slow"/"slow"/"mid"/"high" などのラベル
        - または "1100"〜"1900" のような PWM 数値文字列
        """
        if not isinstance(speed, str):
            raise InvalidSpeedLevel(f"Speed must be a string, got {type(speed)}")

        # 数値(PWM)指定はそのまま受け入れ（後でクリップ）
        if speed.isdigit():
            return speed
    ...
    def apply_command(self, command: str, speed: Optional[str] = None) -> None:
        ...
        # 数値(PWM)指定かどうか判定
        numeric_pwm: Optional[int] = None
        if spd.isdigit():
            numeric_pwm = int(spd)
            # セーフティのため 1100〜1900 にクリップ
            if numeric_pwm < 1100:
                numeric_pwm = 1100
            elif numeric_pwm > 1900:
                numeric_pwm = 1900
        ...
        if cmd == "stop":
            ...
            ch3 = numeric_pwm if numeric_pwm is not None else THROTTLE_FWD[spd]
            ch1 = 1535
        elif cmd == "forward_left":
            # ch3 = numeric_pwm if numeric_pwm is not None else THROTTLE_FWD[spd]
            # ch1 = 1400
            ch3 = 1700
            ch1 = numeric_pwm if numeric_pwm is not None else YAW_LEFT[spd]
        elif cmd == "forward_right":
            # ch3 = numeric_pwm if numeric_pwm is not None else THROTTLE_FWD[spd]
            # ch1 = 1600
            ch3 = 1700
            ch1 = numeric_pwm if numeric_pwm is not None else YAW_RIGHT[spd]
        elif cmd == "move_backward":
            ch3 = numeric_pwm if numeric_pwm is not None else THROTTLE_REV[spd]
